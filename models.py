...
class QuantConv_DW(QuantHelper):
    ...
    def quant_forward(self, x, quantizer):
        ...
        quantized_w = repeat(quantized_w, 'o i h w -> (o repeat) i h w',
                             repeat=self.prod_dw_in_planes // self.in_planes)
...
class Quant_dwpw(QuantHelper):
    def __init__(self, prod_dw_in_planes, in_planes, out_planes, n_dw_emb, n_pw_emb, n_f_emb, kernel_size=3, stride=1,
                 padding=0, gs=1, decay=0.99, ret_x=False):
        super().__init__()
        self.ret_x = ret_x
...
class QuantBasicBlock(nn.Module):
    ...
    def __init__(self, n_dw_emb, n_pw_emb, n_f_emb, prod_dw_in_planes, in_planes, out_planes, stride=1, gs=1):
        ...
        self.conv1 = Quant_dwpw(prod_dw_in_planes, in_planes, out_planes, n_dw_emb, n_pw_emb, n_f_emb, stride=stride,
                                padding=1, gs=gs, ret_x=False)

        self.prod_dw_in_planes = self.conv1.get_groups()
        self.conv2 = Quant_dwpw(self.prod_dw_in_planes, out_planes, out_planes * QuantBasicBlock.expansion, n_dw_emb,
                                n_pw_emb, n_f_emb, stride=stride, padding=1, gs=gs, ret_x=False)
...
class QuantNet(nn.Module):
    def __init__(self, in_channels, n_dw_emb, n_pw_emb, n_f_emb, block, num_blocks, num_classes, gs, out_planes=3):
        ...
        self.head = nn.Linear(out_planes * block.expansion, num_classes)
    ...
    def _make_layer(self, block, out_planes, num_blocks, stride, n_dw_emb, n_pw_emb, gs, n_f_emb):
        ...
        for i in range(len(strides)):
            block_net.add_module(f'QuantRes{i + 1}',
                                 block(n_dw_emb, n_pw_emb, n_f_emb, self.prod_dw_in_planes, self.inplanes, out_planes,
                                       strides[i], gs))
