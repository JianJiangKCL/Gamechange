...
class ModeScheduler(pl.Callback):

    def __init__(self, freeze_start_epoch):
        super().__init__()
        self.freeze_start_epoch = freeze_start_epoch
        print('-----------AQ mode scheduler----------')
        self.layers_eval = []

    @staticmethod
    def freeze_module_(module, is_quant=True):
        module.requires_grad_(False)
        module.eval()
        if is_quant:
            module.straight_mode_()

    def on_train_epoch_start(self, trainer: "pl.Trainer", pl_module: AqMethod) -> None:
        if pl_module.current_epoch == self.freeze_start_epoch:
            print('-----------Freeze layers----------')
            self.freeze_module_(pl_module.backbone.conv1, is_quant=False)
            self.freeze_module_(pl_module.backbone.layer1)
            self.freeze_module_(pl_module.backbone.conv1)
            self.layers_eval.append(pl_module.backbone.layer1)

    def on_train_start(self, trainer: "pl.Trainer", pl_module: AqMethod) -> None:
        for layer in self.layers_eval:
            layer.eval()
